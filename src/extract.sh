...
# # List all .pcap files and loop over them
# for file_to_read in $directory/data/*.pcap; do
    # # Replace the extension .pcap with .csv for the output file
    # file_to_write="${file_to_read//.pcap/.csv}"

    # # Execute Tshark command on each file
    ...
    # # Search for Message Type ID in the file
    # searchForMessageTypeIdInFile "$directory/data/tsharkOutput/$file_to_write" $message_type_id 10

    # # Check if the message type ID was found and process accordingly
...
#batshark -r $file_to_read --disable-protocol wsmp -Tfields -Eseparator=, -e frame.time_epoch -e data.data > $directory/data/tsharkOutput/$file_to_write
